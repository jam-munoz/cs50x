...
void tabulate(void)
{
	...
	for (voter = 0; voter < voter_count; )
	{
		candi_index = 0;
		while (candi_index != preferences[voter][call])
		{
			candi_index++;
		}
		if (candidates[candi_index].eliminated == false)
		{
			candidates[candi_index].votes++;
			call = 0;
			voter++;
		}
		else
